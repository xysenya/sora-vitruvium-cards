...
function saveConfig(config) {
  try {
    ...
    console.error('Не удалось сохранить deploy-config.json:', e.message);
  }
}
...
async function main() {
  ...
  try {
    ...
    try {
      runRootGit(`git commit -m "Update Vitruvium App - ${new Date().toLocaleString('ru-RU')}"`);
    } catch (e) {
