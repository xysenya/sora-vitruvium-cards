...
export function splitHTMLByLength(html: string, pageLimits: number[]): string[] {
  ...
  while (i < len) {
    // Check if we are at an atomic inline-card-icon block
    if (html.slice(i, i + 35).includes('inline-card-icon')) {
      const endIdx = html.indexOf('</span>', i);
      if (endIdx !== -1) {
        const fullIconHtml = html.slice(i, endIdx + 7);
        currentChunkHtml += fullIconHtml;
        currentChunkTextLength += 2; // Treat as ~2 characters (like an emoji)
        i = endIdx + 7;
        continue;
      }
    }
  ...
  const cleanedChunks = chunks.filter(c => {
    ...
    // Check if it has empty tags that actually do something like <br>, <hr>, <img> or inline icons
    return /<br\s*\/?>/i.test(c) || /<hr\s*\/?>/i.test(c) || /<img\s*[^>]*>/i.test(c) || /inline-card-icon/i.test(c) || /<svg\b/i.test(c);
  });
...
/**
 * Tokenizes HTML into individual tags, atomic inline icons, and words with trailing whitespaces.
 */
export function tokenizeHTML(html: string): string[] {
  ...
  while (i < len) {
    // Treat entire inline-card-icon as a single indivisible token so it is never broken across pages
    if (html.startsWith('<span class="inline-card-icon"', i) || html.startsWith("<span class='inline-card-icon'", i)) {
      const endIdx = html.indexOf('</span>', i);
      if (endIdx !== -1) {
        tokens.push(html.slice(i, endIdx + 7));
        i = endIdx + 7;
        continue;
      }
    }
...
export function getCardPages(card: Card, rarities?: RarityDefinition[]): string[] {
  ...
  const createCardMirror = (pageIdx: number, hasMultiplePages: boolean) => {
    ...
    const leftText = card.footerTextLeft !== undefined && card.footerTextLeft !== '' 
      ? card.footerTextLeft 
      : (card.footerText !== undefined && card.footerText !== '' ? card.footerText : '• SORA •');
  ...
  const checkTokenFit = (
    ...
    for (let i = 0; i < count; i++) {
      ...
      } else if (token.includes('inline-card-icon')) {
        // Self-contained inline card icon token, do not treat as opening tag
    ...
    const hasVisualElements = /<img\s*[^>]*>/i.test(fullHtml) || /<hr\s*\/?>/i.test(fullHtml) || /<br\s*\/?>/i.test(fullHtml) || /inline-card-icon/i.test(fullHtml) || /<svg\b/i.test(fullHtml);
...
export function getCardArtBgState(card: Card) {
  const hasArt = Boolean(card.artUrl && !card.fullIllustration);
  const isBoth = Boolean(hasArt && card.artAsDescriptionBg && card.artAsStatsBg);
  const isDescOnly = Boolean(hasArt && card.artAsDescriptionBg && !card.artAsStatsBg);
  const isStatsOnly = Boolean(hasArt && card.artAsStatsBg && !card.artAsDescriptionBg);
  return { hasArt, isBoth, isDescOnly, isStatsOnly };
}
