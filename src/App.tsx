...
import { getCardPages, getIllustrationHeight, isStatsOnSecondPage, shouldRenderCardStats } from './utils';
...
const AVAILABLE_ICONS = [
  // Бой и защита
  'Sword', 'Swords', 'Shield', 'ShieldCheck', 'ShieldX', 'ShieldQuestion', 'ShieldAlert', 'Dagger', 'Axe', 'Hammer', 'Pickaxe', 'Target', 'Crosshair', 'Bomb',
  ...
  'Backpack', 'Tent', 'Bed', 'Compass', 'Map', 'MapPin', 'Key', 'KeyRound', 'Lock', 'Unlock', 'Anchor', 'Chain', 'Lantern', 'Flag', 'Hourglass', 'Clock', 'Binoculars',
...
const getIconCategory = (iconName: string): string => {
  const weapons = ['Sword', 'Swords', 'Shield', 'ShieldCheck', 'ShieldX', 'ShieldQuestion', 'ShieldAlert', 'Dagger', 'Axe', 'Hammer', 'Pickaxe', 'Target', 'Crosshair', 'Bomb'];
  ...
  const gear = ['Backpack', 'Tent', 'Bed', 'Compass', 'Map', 'MapPin', 'Key', 'KeyRound', 'Lock', 'Unlock', 'Anchor', 'Chain', 'Lantern', 'Flag', 'Hourglass', 'Clock', 'Binoculars'];
    ...
    <div className="grid grid-cols-3 w-full items-center mt-1 border-t border-stone-200 pt-1 shrink-0 text-[6px] font-mono select-none">
      {showLeft && (
        isEditable && onUpdateField ? (
          <div className={`${leftSpan} flex items-center min-w-0 pr-0.5`}>
            <input
              type="text"
              value={card.footerTextLeft !== undefined ? card.footerTextLeft : (card.footerText !== undefined ? card.footerText : '')}
              onChange={(e) => {
                onUpdateField('footerTextLeft', e.target.value);
                onUpdateField('footerText', e.target.value);
              }}
              ...
              maxLength={80}
              className="w-full bg-transparent border border-transparent hover:border-stone-400/50 hover:bg-stone-500/10 focus:border-amber-500 focus:bg-white/80 focus:ring-1 focus:ring-amber-400/40 focus:outline-none rounded px-0.5 py-0 text-[6px] font-mono uppercase tracking-tight text-left truncate transition-all cursor-text placeholder:text-stone-300 placeholder:normal-case"
              style={{ color: card.customFooterLeftColor || card.customFooterTextColor || '#a8a29e' }}
            />
          </div>
        ) : (
          <span 
            className={`${leftSpan} truncate text-stone-400 uppercase`}
      ...
      {showMiddle && (
        isEditable && onUpdateField ? (
          <div className={`${middleSpan} flex items-center justify-center min-w-0 px-0.5`}>
            <input
              type="text"
              value={card.footerTextMiddle !== undefined ? card.footerTextMiddle : ''}
              onChange={(e) => onUpdateField('footerTextMiddle', e.target.value)}
              onClick={(e) => e.stopPropagation()}
              onPointerDown={(e) => e.stopPropagation()}
              onMouseDown={(e) => e.stopPropagation()}
              onKeyDown={(e) => {
              ...
              maxLength={80}
              className="w-full bg-transparent border border-transparent hover:border-stone-400/50 hover:bg-stone-500/10 focus:border-amber-500 focus:bg-white/80 focus:ring-1 focus:ring-amber-400/40 focus:outline-none rounded px-0.5 py-0 text-[6px] font-mono uppercase tracking-tight text-center truncate transition-all cursor-text placeholder:text-stone-300 placeholder:normal-case"
              style={{ color: card.customFooterMiddleColor || '#a8a29e' }}
            />
          </div>
        ) : (
          <span 
            className={`${middleSpan} truncate text-stone-400 uppercase px-1`}
            style={{ color: card.customFooterMiddleColor || undefined }}
            title={middleText}
          >
            {middleText}
          </span>
      ...
      {showRight && (
        isEditable && onUpdateField ? (
          <div className={`${rightSpan} flex items-center justify-end min-w-0 pl-0.5`}>
            <input
              type="text"
              value={card.footerTextRight !== undefined ? card.footerTextRight : (card.cardNumber !== undefined ? card.cardNumber : '')}
              onChange={(e) => {
                onUpdateField('footerTextRight', e.target.value);
              ...
              maxLength={80}
              className="w-full bg-transparent border border-transparent hover:border-stone-400/50 hover:bg-stone-500/10 focus:border-amber-500 focus:bg-white/80 focus:ring-1 focus:ring-amber-400/40 focus:outline-none rounded px-0.5 py-0 text-[6px] font-mono uppercase tracking-tight text-right truncate transition-all cursor-text placeholder:text-stone-300 placeholder:normal-case"
              style={{ color: card.customFooterRightColor || card.customCardNumberColor || '#a8a29e' }}
            />
          </div>
        ) : (
          <span 
            className={`${rightSpan} truncate text-stone-400 uppercase`}
            style={{ color: card.customFooterRightColor || card.customCardNumberColor || undefined }}
            title={rightText}
          >
            {rightText}
          </span>
...
function RichTextEditor({ value, onChange, cardId, className, style, placeholder, isCardPreview }: RichTextEditorProps) {
    ...
    setTimeout(() => {
      ...
      try {
        ...
        if (editorRef.current && editorRef.current.contains(range.commonAncestorContainer)) {
          ...
          setToolbarCoords({
            top: rect.top - 8, // Position slightly above selection
  ...
  useEffect(() => {
    const handleGlobalPointerDown = (e: MouseEvent | PointerEvent) => {
      ...
      if (editorRef.current.contains(target) || (toolbar && toolbar.contains(target))) {
        return;
      }
    ...
    if (html) {
      ...
      const cleanNode = (node: Node): Node | null => {
        ...
        if (node.nodeType === Node.ELEMENT_NODE) {
          ...
          const allowedTextFormats = ['b', 'strong', 'i', 'em', 'u', 's', 'strike', 'del', 'br'];
          const allowedBlocks = ['p', 'div', 'li'];

          if (allowedTextFormats.includes(tagName)) {
...
export default function App() {
  ...
  useEffect(() => {
    document.title = "Мастерская карт Витрувий";
  ...
  const requestAlert = (
    ...
    const lang = appSettings.language;
    let title = lang === 'en' ? 'VITRUVIUM • BROADCAST' : 'ВИТРУВИЙ • ВЕЩАНИЕ';
    let message = '';
  ...
  const handleCopyPrompt = () => {
    const templatePrompt = `Привет! Помоги мне создать карту / набор карт для НРИ "Витрувий". Я это делаю через веб-приложение, которое поддерживает импорт json массивов. Мне нужно создать карточки для способностей, оружия, доспехов, зелий и предметов.
...
Пожалуйста, сгенерируй для меня интересный тематический набор из 3-5 карточек по этой структуре. Верни только чистый JSON-массив без лишних слов.`;
      ...
      return (
        <div className="w-full h-full bg-white flex flex-col overflow-hidden relative">
          ...
          <div className="flex-1 p-2.5 overflow-hidden flex flex-col justify-between"
            ...
            <div
              className={`leading-relaxed text-justify overflow-hidden pr-0.5 flex-1 min-h-0 break-words font-body ${getContentFontSize(card.content || '')}`}
              style={{
    ...
    return (
      <div className="w-full h-full bg-gradient-to-b from-stone-900 via-stone-800 to-stone-950 flex flex-col items-center justify-start p-4 relative">
        ...
        <div className="text-amber-500/40 text-[8px] font-serif tracking-widest uppercase mt-4 z-10">
          • {isEn ? 'VITRUVIUM' : 'ВИТРУВИЙ'} •
        </div>

        {/* Medallion */}
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-24 h-24 rounded-full border-2 border-amber-600/40 bg-stone-950/80 flex items-center justify-center shadow-lg z-10">
          <div 
          ...
          <svg 
            viewBox="0 0 512 257" 
            className="w-14 h-auto text-amber-500 fill-current"
          >
            <g>
              <path d="M0 0L176 81.5L256 257L83 174L0 0Z" fill="currentColor" fillRule="evenodd" />
              <path d="M0 0L176 81.5L256 257L83 174L0 0Z" fill="currentColor" fillRule="evenodd" transform="matrix(-1 0 0 1 512 0)" />
            </g>
          </svg>
        </div>
      </div>
  ...
  const handleDownloadDefaultCardBackSvg = () => {
    const brandName = isEn ? 'VITRUVIUM' : 'ВИТРУВИЙ';
  ...
  const handleDownloadPNGDeck = () => {
    ...
    requestConfirmation('download', async () => {
      ...
      try {
        ...
        link.download = isEn ? 'My Vitruvium Deck.zip' : 'Моя колода ВИТРУВИЙ.zip';
        document.body.appendChild(link);
        link.click();
        document.body.removeChild(link);
        ...
        <div className="max-w-7xl mx-auto flex flex-col xl:flex-row xl:items-center justify-between gap-3 xl:gap-4">
          ...
          <div className="flex items-center justify-between w-full xl:w-auto gap-3">
            <div className="flex items-center gap-3 min-w-0">
              <div className="w-10 h-10 rounded bg-amber-600 flex items-center justify-center text-stone-950 shadow-inner shrink-0">
                <svg 
                  viewBox="0 0 512 257" 
                  className="w-7 h-auto text-stone-950 fill-current"
                >
                  <g>
                    <path d="M0 0L176 81.5L256 257L83 174L0 0Z" fill="currentColor" fillRule="evenodd" />
                    <path d="M0 0L176 81.5L256 257L83 174L0 0Z" fill="currentColor" fillRule="evenodd" transform="matrix(-1 0 0 1 512 0)" />
                    ...
                    return (
                      ...
                      >
                        ...
                        >
                          ...
                          {card.stats && card.stats.length > 0 && shouldRenderCardStats(card, pageIdx, rarities) && (
                            <div className="grid grid-cols-2 border-b border-stone-950 shrink-0 divide-x divide-stone-200"
                                 style={{ backgroundColor: card.customStatsBgColor || '#f5f5f4' }}
                            >
                              {card.stats.map((stat) => (
                                <div key={stat.id} className="flex flex-col justify-center px-1.5 py-0.5 leading-none min-w-0">
                                  <input
                                    type="text"
                                    value={stat.label || ''}
                                    onChange={(e) => handleUpdateCardStatById(card.id, stat.id, e.target.value, stat.value)}
                                    placeholder={t.propLabelPlaceholder}
                                    className="text-[7px] uppercase font-bold tracking-wider leading-none bg-transparent border border-transparent hover:border-stone-400/50 hover:bg-stone-200/50 focus:border-amber-500 focus:bg-white focus:outline-none rounded-sm px-0.5 py-0 h-[12px] transition-all w-full cursor-text"
                                    style={{ color: card.customStatsTextColor || '#78716c', opacity: card.customStatsTextColor ? 0.8 : 1 }}
                                    title={t.clickToEditParamTooltip}
                                  />
                                  <input
                                    type="text"
                                    value={stat.value || ''}
                                    onChange={(e) => handleUpdateCardStatById(card.id, stat.id, stat.label, e.target.value)}
                                    placeholder={t.propValuePlaceholder}
                                    className={`font-mono font-bold tracking-tight leading-none bg-transparent border border-transparent hover:border-stone-400/50 hover:bg-stone-200/50 focus:border-amber-500 focus:bg-white focus:outline-none rounded-sm px-0.5 py-0 h-[15px] transition-all w-full cursor-text ${getStatValueFontSize(stat.value || '—')}`}
                                    style={{ color: card.customStatsTextColor || '#1c1917' }}
                          ...
                          {/* Rich Description Block */}
                          {(!card.fullIllustration || pageIdx > 0) && (
                            <div className="flex-1 p-2.5 overflow-hidden flex flex-col justify-between"
                                 style={{ backgroundColor: card.customContentBgColor || 'rgba(250, 250, 249, 0.5)' }}>
                              <div className="flex-1 min-h-0 flex flex-col overflow-hidden">
                                <RichTextEditor
                                  ...
                                  isCardPreview={true}
                                  className={`rich-editor-preview pr-0.5 flex-1 min-h-0 break-words font-body select-text cursor-text leading-relaxed text-justify outline-none ${card.enableTextOverflow ? 'overflow-hidden' : 'overflow-y-auto'}`}
                                  style={{
                                    color: card.customContentColor || '#1c1917',
                                    fontSize: card.fontSize !== undefined ? `${card.fontSize}px` : '9px',
                ...
                {getCardPages(selectedCard, rarities).map((pageContent, pageIdx, pagesArr) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {/* Optional Custom Stats Table */}
                      {selectedCard.stats && selectedCard.stats.length > 0 && shouldRenderCardStats(selectedCard, pageIdx, rarities) && (
                        <div className="grid grid-cols-2 border-b border-stone-950 shrink-0 divide-x divide-stone-200"
                             style={{ backgroundColor: selectedCard.customStatsBgColor || '#f5f5f4' }}
                        >
                          {selectedCard.stats.map((stat) => (
                            <div key={stat.id} className="flex flex-col justify-center px-1.5 py-0.5 leading-none min-w-0">
                              <input
                                type="text"
                                value={stat.label || ''}
                                onChange={(e) => handleUpdateStat(stat.id, e.target.value, stat.value)}
                                placeholder={t.propLabelPlaceholder}
                                className="text-[7px] uppercase font-bold tracking-wider leading-none bg-transparent border border-transparent hover:border-stone-400/50 hover:bg-stone-200/50 focus:border-amber-500 focus:bg-white focus:outline-none rounded-sm px-0.5 py-0 h-[12px] transition-all w-full cursor-text"
                                style={{ color: selectedCard.customStatsTextColor || '#78716c', opacity: selectedCard.customStatsTextColor ? 0.8 : 1 }}
                                title={t.clickToEditParamTooltip}
                              />
                              <input
                                type="text"
                                value={stat.value || ''}
                                onChange={(e) => handleUpdateStat(stat.id, stat.label, e.target.value)}
                                placeholder={t.propValuePlaceholder}
                                className={`font-mono font-bold tracking-tight leading-none bg-transparent border border-transparent hover:border-stone-400/50 hover:bg-stone-200/50 focus:border-amber-500 focus:bg-white focus:outline-none rounded-sm px-0.5 py-0 h-[15px] transition-all w-full cursor-text ${getStatValueFontSize(stat.value || '—')}`}
                                style={{ color: selectedCard.customStatsTextColor || '#1c1917' }}
                                title={t.clickToEditValueTooltip}
                              />
                            </div>
                          ))}
                      ...
                      {/* Rich Description Block */}
                      {(!selectedCard.fullIllustration || pageIdx > 0) && (
                        <div className="flex-1 p-2.5 overflow-hidden flex flex-col justify-between"
                             style={{ backgroundColor: selectedCard.customContentBgColor || 'rgba(250, 250, 249, 0.5)' }}>
                          <div className="flex-1 min-h-0 flex flex-col overflow-hidden">
                            <RichTextEditor
                              ...
                              placeholder={t.cardDescPlaceholder}
                              isCardPreview={true}
                              className={`rich-editor-preview pr-0.5 flex-1 min-h-0 break-words font-body select-text cursor-text leading-relaxed text-justify outline-none ${selectedCard.enableTextOverflow ? 'overflow-hidden' : 'overflow-y-auto'}`}
                              style={{
                        ...
                        selectedCard.stats.map(stat => (
                          <div key={stat.id} className="flex items-center gap-1.5">
                            <input
                              ...
                              onChange={(e) => handleUpdateStat(stat.id, e.target.value, stat.value)}
                              className="w-1/2 px-2 py-1 bg-stone-50 dark:bg-stone-800 border border-stone-200 dark:border-stone-700 rounded text-[10px] text-stone-600 dark:text-stone-300 font-bold focus:bg-white dark:focus:bg-stone-750"
                            />
                            <input
                              type="text"
                              value={stat.value}
                              ...
                              onChange={(e) => handleUpdateStat(stat.id, stat.label, e.target.value)}
                              className="w-1/2 px-2 py-1 bg-stone-50 dark:bg-stone-800 border border-stone-200 dark:border-stone-700 rounded text-[10px] text-stone-800 dark:text-stone-100 focus:bg-white dark:focus:bg-stone-750"
                            />
                            <button
                              onClick={() => handleRemoveStatRow(stat.id)}
                              className="text-stone-400 hover:text-red-500 dark:hover:text-red-400 p-1 cursor-pointer"
                  ...
                  <div className="flex flex-col gap-2 border-t border-stone-100 dark:border-stone-800 pt-3">
                    <div className="flex justify-between items-center">
                      ...
                      <div className="flex items-center gap-1 select-none">
                        <span className="text-[10px] text-stone-400 font-mono">{t.fontSizeTitle}</span>
                        <button
                          type="button"
                          onClick={() => {
                          ...
                          <div className="flex flex-col gap-4 pl-1">
                            ...
                            <div className="grid grid-cols-[auto_1fr_auto] gap-x-2 gap-y-1 items-center">
                              ...
                              <input
                                ...
                                onChange={(e) => handleUpdateMultipleFields({ footerTextLeft: e.target.value, footerText: e.target.value })}
                                placeholder={t.footerLeftPlaceholder}
                                maxLength={80}
                                disabled={!!selectedCard.hideFooterLeft}
                                className={`px-2.5 py-1.5 bg-stone-50 dark:bg-stone-800 border border-stone-200 dark:border-stone-700 rounded text-xs focus:outline-none focus:ring-1 focus:ring-stone-400 focus:bg-white dark:focus:bg-stone-750 text-stone-700 dark:text-stone-200 w-full ${selectedCard.hideFooterLeft ? 'opacity-50 select-none' : ''}`}
                              />
                              ...
                              <div className="flex items-center gap-1 shrink-0">
                                <label className={`w-8 h-8 rounded border border-stone-200 dark:border-stone-700 cursor-pointer flex items-center justify-center bg-white dark:bg-stone-800 shadow-sm hover:bg-stone-50 dark:hover:bg-stone-700 transition-colors ${selectedCard.hideFooterLeft ? 'opacity-50 pointer-events-none' : ''}`} title={t.chooseColorTitle}>
                                  <input
                                    ...
                                    onChange={(e) => handleUpdateMultipleFields({ customFooterLeftColor: e.target.value, customFooterTextColor: e.target.value })}
                                    className="sr-only"
                                    disabled={!!selectedCard.hideFooterLeft}
                                  />
                                  <div 
                                ...
                                {(selectedCard.customFooterLeftColor || selectedCard.customFooterTextColor) && !selectedCard.hideFooterLeft && (
                                  <button
                                    onClick={() => handleUpdateMultipleFields({ customFooterLeftColor: undefined, customFooterTextColor: undefined })}
                            ...
                            <div className="grid grid-cols-[auto_1fr_auto] gap-x-2 gap-y-1 items-center mt-1">
                              ...
                              <input
                                ...
                                onChange={(e) => handleUpdateCardField('footerTextMiddle', e.target.value)}
                                placeholder={t.footerMiddlePlaceholder}
                                maxLength={80}
                                disabled={!!selectedCard.hideFooterMiddle}
                                className={`px-2.5 py-1.5 bg-stone-50 dark:bg-stone-800 border border-stone-200 dark:border-stone-700 rounded text-xs focus:outline-none focus:ring-1 focus:ring-stone-400 focus:bg-white dark:focus:bg-stone-750 text-stone-700 dark:text-stone-200 w-full ${selectedCard.hideFooterMiddle ? 'opacity-50 select-none' : ''}`}
                              />
                            ...
                            <div className="grid grid-cols-[auto_1fr_auto] gap-x-2 gap-y-1 items-center mt-1">
                              ...
                              <input
                                ...
                                onChange={(e) => handleUpdateMultipleFields({ footerTextRight: e.target.value, cardNumber: e.target.value })}
                                placeholder={`#${selectedCard.id.slice(-4)}`}
                                maxLength={40}
                                disabled={!!selectedCard.hideFooterRight}
                                className={`px-2.5 py-1.5 bg-stone-50 dark:bg-stone-800 border border-stone-200 dark:border-stone-700 rounded text-xs focus:outline-none focus:ring-1 focus:ring-stone-400 focus:bg-white dark:focus:bg-stone-750 text-stone-700 dark:text-stone-200 font-mono w-full ${selectedCard.hideFooterRight ? 'opacity-50 select-none' : ''}`}
                              />
                ...
                <div className="flex flex-col gap-4">
                  ...
                  {selectedCard.artUrl && (
                    <div className="flex flex-col gap-3 bg-stone-50 dark:bg-stone-800/70 p-2.5 rounded border border-stone-200 dark:border-stone-700">
                      ...
                      <label className="flex items-center gap-2 text-xs text-stone-700 dark:text-stone-300 cursor-pointer select-none bg-stone-100/80 dark:bg-stone-800 hover:bg-stone-200/60 dark:hover:bg-stone-750 p-1.5 rounded transition-colors border border-stone-200/60 dark:border-stone-700">
                        <input
                          type="checkbox"
          ...
          const renderSinglePrintCardFront = (item: any) => {
            ...
            return (
              ...
              >
                ...
                {card.stats && card.stats.length > 0 && shouldRenderCardStats(card, pageIdx, rarities) && (
                  <div className="grid grid-cols-2 border-b border-stone-950 shrink-0 divide-x divide-stone-200" 
                       style={{ backgroundColor: card.customStatsBgColor || '#f5f5f4', WebkitPrintColorAdjust: 'exact', printColorAdjust: 'exact' }}>
                    {card.stats.map(stat => (
                      <div key={stat.id} className="flex flex-col justify-center px-1.5 py-0.5 leading-none min-w-0">
                        <span className="text-[7px] uppercase font-bold tracking-wider leading-none px-0.5 h-[12px] break-words whitespace-normal"
                              style={{ color: card.customStatsTextColor || '#78716c', opacity: card.customStatsTextColor ? 0.8 : 1 }}>
                          {stat.label}
                        </span>
                        <span className={`font-mono font-bold tracking-tight leading-none px-0.5 h-[15px] break-words whitespace-normal ${getStatValueFontSize(stat.value || '—')}`}
                              style={{ color: card.customStatsTextColor || '#1c1917' }}>
                          {stat.value}
                        </span>
                      </div>
                ...
                {(!card.fullIllustration || pageIdx > 0) && (
                  <div className="flex-1 p-2.5 overflow-hidden flex flex-col justify-between" 
                       style={{ backgroundColor: card.customContentBgColor || 'rgba(250, 250, 249, 0.5)', WebkitPrintColorAdjust: 'exact', printColorAdjust: 'exact' }}>
                    <div
                      className={`leading-relaxed text-justify ${card.enableTextOverflow ? 'overflow-hidden' : 'overflow-y-auto'} flex-1 min-h-0 break-words font-body ${getContentFontSize(card.content || '')}`}
                      style={{
                        color: card.customContentColor || '#1c1917',
                        fontSize: card.fontSize !== undefined ? `${card.fontSize}px` : '9px'
                      }}
          ...
          <div className="bg-stone-900 w-full max-w-md rounded-xl shadow-2xl border-2 border-amber-600/50 overflow-hidden flex flex-col relative z-10 animate-in fade-in zoom-in-95 duration-200">
            ...
            <div className="px-4 py-3 border-b border-stone-800 flex items-center gap-3 bg-stone-950">
              ...
              <h3 className="font-serif font-bold text-sm tracking-wide text-amber-400 uppercase">
                {confirmModal.title || t.vitruviusConfirmModalDefaultTitle || (isEn ? 'VITRUVIUM • CONFIRMATION' : 'ВИТРУВИЙ • ПОДТВЕРЖДЕНИЕ')}
              </h3>
            </div>
                ...
                <div className="flex flex-col gap-2 bg-stone-50 dark:bg-stone-850 p-3 rounded-lg border border-stone-200/80 dark:border-stone-750">
                  ...
                  <div className="grid grid-cols-4 gap-2">
                    {COLOR_ELEMENT_DEFINITIONS.map(elem => {
                      const colorVal = (selectedCard as any)[elem.key] || (elem.key.includes('Title') || elem.key.includes('Subtitle') ? '#ffffff' : (elem.key.includes('Text') ? '#1c1917' : (elem.key.includes('Stats') || elem.key.includes('Content') ? '#f5f5f4' : getCardColors(selectedCard).primary)));
                      return (
      ...
      <div id="png-export-container" className="fixed top-[-9999px] left-[-9999px] flex flex-col gap-10" style={{ pointerEvents: 'none' }}>
          ...
          >
            ...
            {(!card.fullIllustration || pageIdx > 0) && (
              <div className="flex-1 p-2.5 overflow-hidden flex flex-col justify-between"
                   style={{ backgroundColor: card.customContentBgColor || 'rgba(250, 250, 249, 0.5)' }}>
                <div
                  className={`leading-relaxed text-justify ${card.enableTextOverflow ? 'overflow-hidden' : 'overflow-y-auto'} pr-0.5 flex-1 min-h-0 break-words font-body ${getContentFontSize(card.content || '')}`}
                  style={{
                    color: card.customContentColor || '#1c1917',
                    fontSize: card.fontSize !== undefined ? `${card.fontSize}px` : '9px'
                  }}
        ...
        {cardBackEnabled && (selectedCard || cards[0]) && (
          <div
            ...
            style={{
              width: `${(selectedCard || cards[0])?.width || 63}mm`,
              height: `${(selectedCard || cards[0])?.height || 88}mm`,
              boxSizing: 'border-box',
